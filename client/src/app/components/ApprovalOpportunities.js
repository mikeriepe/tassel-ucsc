    ...
    <React.Fragment>
      <TableRow>
        <TableCell className='data-cell' padding='checkbox'>
          <Checkbox value={row.eventid} onChange={handleSelect}/>
        </TableCell>
        <TableCell className='data-cell' padding='checkbox'>
          <IconButton
        ...
        <TableCell className='data-cell'>
          <div style={{display: 'flex',
            flexDirection: 'row',
            color: opportunityStatusToColor(row.active)}}>
            <FiberManualRecordIcon sx={{
              fontSize: '1em',
              paddingTop: '.21rem',
              paddingRight: '.21rem'}}/>
            <div>{opportunityStatusToText(row.active)}</div>
          </div>
        </TableCell>
      </TableRow>
      <TableRow>
...
export default function ApprovalOpportunities() {
  ...
  const sortOpps = (json, sortBy, reset) => {
    if (reset === true) {
      setOpps(json.sort(function(a, b) {
        /* eslint-disable-next-line max-len */
        return opportunityStatusToText(a.active) > opportunityStatusToText(b.active) ? -1 : 1;
      }));
      ...
      if (sortCreatorOrder === '') {
        setOpps(json.sort(function(a, b) {
          const first = profiles.find((profile) =>
            profile.profileid == a.usersponsors.creator);
          const second = profiles.find((profile) =>
            profile.profileid == b.usersponsors.creator);
  ...
  const handleStatusAction = (event) => {
    ...
    const opportunities = selected.map((opportunity) => {
      const info = opps.find((opp) => opp.eventid == opportunity);
      info.active = status;
      return info;
    });
    setLoading(true);
                  ...
                  return (
                    <Row
                      ...
                      profile={profiles.find((profile) =>
                        profile.profileid == opp.usersponsors.creator)}
