// PLEASE READ: THIS TESTING FILE REQUIRES THAT AT LEAST 1 organizationType IS IN THE DB
...
test(`GET all organizationTypes`, async () => {
    // GET A JWT FIRST
    ...
    await supertest(app).get('/api/getOrganizationTypes')
        ...
        .then((response) =>{
            // console.log(response.body);
            expect(response.body[0].organizationtypeid !== undefined).toEqual(true);
